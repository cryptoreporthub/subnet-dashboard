"""FastAPI routes for live signals, alerts, and WebSocket streaming."""
...
async def _refresh_and_broadcast() -> Dict[str, Any]:
    """Generate signals, run alert checks, push to WebSocket clients."""
    result = await asyncio.to_thread(generate_live_signals, True)
    engine = _get_alerts()
    system = await asyncio.to_thread(engine.check_system_alerts)
    signal_alerts = await asyncio.to_thread(engine.record_signal_changes, result.get("changed_signals") or [])
    hub = get_signal_hub()
...
async def api_signals(
    ...
    if refresh:
        ...
        if subnet_id is not None:
            signals = store.query(subnet_id=subnet_id)
        else:
            signals = store.latest_all()
        result = {"meta": {"count": len(signals), "changed": 0}}
    if subnet_id is not None and refresh:
    ...
    try:
        ...
        while True:
            ...
            if msg.strip().lower() in ("refresh", "ping"):
                result = await _refresh_and_broadcast()
                await websocket.send_json(
                    {"type": "signals", "data": {"signals": result.get("signals", []), "meta": result.get("meta")}}
                )
            else:

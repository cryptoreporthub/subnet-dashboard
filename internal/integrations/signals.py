...
def _fetch_desearch_snippet() -> Optional[Dict[str, Any]]:
    """One-shot web sentiment via DeSearch when keyed."""
    from internal.integrations.clients import desearch_ai_summary

    row = desearch_ai_summary("Bittensor subnet market sentiment", tools=["web"])
    if not row:
        return None
    return {
        "netuid": 22,
...
def build_macro_signals() -> Dict[str, Any]:
    ...
    for fn in (
        _fetch_synth_btc,
        _fetch_numinous_leaderboard,
        _fetch_desearch_snippet,
        _fetch_readyai_sample,
    ):
        try:
            row = fn()
            if row:

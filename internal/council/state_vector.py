...
# Scoring helpers
# ---------------------------------------------------------------------------
def score_subnet_for_hour(state_vector: Dict[str, Any]) -> float:
    """Short-horizon score emphasizing momentum and immediate signals."""
    if not state_vector:
        return 0.0
    metrics = state_vector.get("metrics", {})
    impact = state_vector.get("signal_impact", {})
    hot = state_vector.get("hot", {})
    sell = state_vector.get("sell", {})

    score = 0.0
    score += float(metrics.get("price_change_24h", 0) or 0) * 1.5
    score += float(metrics.get("emission", 0) or 0) * 2.0
    score += float(metrics.get("apy", 0) or 0) * 0.3
    score += float(impact.get("net_predicted_pct", 0) or 0) * 1.2
    if hot.get("active"):
        score += hot.get("score", 0) * 2.0
    if sell.get("active"):
        score -= sell.get("score", 0) * 3.0
    return round(score, 2)


def score_subnet_for_day(state_vector: Dict[str, Any]) -> float:
    """Longer-horizon score emphasizing yield, trend, and lower volatility."""
    if not state_vector:
        return 0.0
    metrics = state_vector.get("metrics", {})
    impact = state_vector.get("signal_impact", {})
    ...
    score = 0.0
    score += float(metrics.get("price_change_7d", 0) or 0) * 0.8
    score += float(metrics.get("price_change_30d", 0) or 0) * 0.4
    score += float(metrics.get("apy", 0) or 0) * 1.0
    score += float(metrics.get("emission", 0) or 0) * 1.5
    score += float(impact.get("net_predicted_pct", 0) or 0) * 0.8
    if hot.get("active"):
        score += hot.get("score", 0) * 1.5
    if sell.get("active"):
        score -= sell.get("score", 0) * 2.5
    return round(score, 2)

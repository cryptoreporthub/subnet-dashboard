...
def _merge_into_registry(live: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Start from registry (display metadata: name, description, logo) and overlay
    live on-chain numeric fields so templates keep working while numbers go live."""
    by_netuid = {}
    for r in live:
        n = _netuid_of(r)
        if n is not None:
...
def _fetch_chain_data() -> Optional[List[Dict[str, Any]]]:
    """Fetch on-chain subnet data in a bounded worker thread. Returns None on any
    failure OR if it exceeds SYNC_TIMEOUT_SECONDS (so the caller never blocks)."""
    result: Dict[str, Any] = {}
    err: Dict[str, Any] = {}
...
def _sync_once() -> bool:
    """Fetch live subnet data and write the merged cache. Returns True on success.
    Never blocks beyond SYNC_TIMEOUT_SECONDS regardless of upstream behavior."""
...
def get_live_subnets() -> List[Dict[str, Any]]:
    """Return merged live subnets from cache (cheap). Triggers a lazy background sync
    if the cache is missing/stale and AUTO_SYNC is on. Never raises, never blocks."""
    try:
...
def _maybe_schedule_sync(cache_data: Optional[Dict[str, Any]]) -> None:
    if not AUTO_SYNC:
        return
...
def live_data_freshness() -> Dict[str, Any]:
    """Freshness payload for /api/data-freshness (audit finding #1)."""
    info = {
        "source": "blockmachine",

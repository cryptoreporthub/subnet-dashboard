...
AUTO_SYNC = os.environ.get("LIVE_SUBNETS_AUTO_SYNC", "true").lower() in ("1", "true", "yes", "on")
...
def _sync_once() -> bool:
    ...
    try:
        ...
        raw = client.get_all_subnet_data()
        if not raw:
            logger.warning("live_subnets sync: empty result from chain client")
            return False

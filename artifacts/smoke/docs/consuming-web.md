# Consuming Smoke Council in web apps

Read `artifacts/smoke/docs/AGENTS.md` first. This guide covers
...
`artifacts/smoke/docs/migrating-web.md` before writing UI.
...
```css
@import "@workspace/smoke/styles.css";
```
...
`node_modules/@workspace/smoke/src/components` to `content`.
...
```tsx
import { Button } from "@workspace/smoke/components/ui/button";
import { cn } from "@workspace/smoke/lib/utils";
import {
  toast,
  useToast,
} from "@workspace/smoke/hooks/use-toast";
```
...
`@workspace/smoke/components/ui/button`. Run the app's typecheck

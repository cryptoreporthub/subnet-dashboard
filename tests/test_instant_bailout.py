...
_ALLOWLIST_PATHS = (
    ...
    "/static/js/mindmap_graph.js",
    "/static/js/api_fetch.js",
    "/static/css/ui.css",
)


def test_bailout_static_allowlist_api_fetch_js_when_inner_wedged():
    app = wrap_instant_bailout(
        _wedged_app,
        get_homepage_html=lambda: None,
        schedule_warm=lambda: None,
    ...
    async def _check():
        async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
            t0 = time.time()
            resp = await client.get("/static/js/api_fetch.js")
            return resp, time.time() - t0
    ...
    assert elapsed < 1.0
    assert "application/javascript" in resp.headers.get("content-type", "")
    assert b"fetchJson" in resp.content


def test_bailout_all_allowlisted_assets_bypass_inner_app():
    app = wrap_instant_bailout(
        _wedged_app,
        get_homepage_html=lambda: None,
        schedule_warm=lambda: None,
    ...
    responses, elapsed = _run(_check())
    assert elapsed < 1.0
    for path, resp in zip(_ALLOWLIST_PATHS, responses):
        assert resp.status_code == 200, path
        assert len(resp.content) > 50, path


def test_bailout_missing_allowlist_file_falls_through_to_inner(tmp_path):
    hit = []
    ...
    async def _check():
        async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
            return await client.get("/static/js/api_fetch.js")

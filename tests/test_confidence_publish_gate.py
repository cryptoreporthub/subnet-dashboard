"""Healthy picks must be able to clear the publish gate after mild RedTeam haircuts."""
...
def test_cold_start_healthy_survives_mild_red_team_haircut():
    ...
    raw = sv._compute_confidence(sn, indicators, experts, total_score=78.0)
    assert raw > publish_gate_fraction()
    sn["confidence"] = raw
...
def test_buy_sell_volume_counts_as_complete():
    ...
    assert conf == pytest.approx(sv._COLD_START_PRIOR, rel=1e-2)

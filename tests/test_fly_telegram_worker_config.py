...
def test_fly_toml_web_vm_shared_cpu_2x_1gb():
    """Web VM: shared-cpu-2x 1GB after post-leak soak on 26e7dec2."""
    fly = _fly_toml()
    assert re.search(
        r'\[\[vm\]\][\s\S]*size = "shared-cpu-2x"[\s\S]*memory = "1gb"[\s\S]*processes = \["web"\]',
        fly,
    )

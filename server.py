...
def _refresh_judge_scores() -> None:
    """Compatibility hook for tests and background refresh jobs."""
    try:
        ...
        if subnets:
            score_all_subnets(subnets)
    except Exception as exc:
        logger.debug("Judge score refresh skipped: %s", exc)


@app.get("/api/judges/sample")
def api_judges_sample():
...
# Judge council routes (full subnet scoring, portfolios, postmortems)
try:
...
except Exception as _ruggers_exc:  # pragma: no cover
    logger.warning("Ruggers watchlist unavailable: %s", _ruggers_exc)

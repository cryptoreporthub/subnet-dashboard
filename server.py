...
def _ensure_background_sync():
    ...
    if app.config["ENABLE_BACKGROUND_SYNC"] and not app.config.get("TESTING"):
        ...
        # Phase 0: Unified scheduler replaces the three competing loops.
...
# ---------------------------------------------------------------------------
# Phase 0: Mindmap conviction decay & hypothesis endpoints
# ---------------------------------------------------------------------------

@app.route("/api/mindmap/nodes", methods=["GET"])
def get_mindmap_nodes():
    """Return active and pruned Mindmap nodes with decay alpha values."""
    include_pruned = request.args.get("include_pruned", "false").lower() == "true"
    result = bridge.get_active_nodes()
    nodes = result["active"]
    if include_pruned:
        nodes = nodes + result["pruned"]
    return jsonify({
        "status": "success",
        "data": {
            "nodes": nodes,
...
@app.route("/api/mindmap/hypotheses", methods=["GET"])
def get_hypotheses():
    """Return pending and resolved hypotheses."""
    include_resolved = request.args.get("include_resolved", "false").lower() == "true"
    hypotheses = bridge._soul_map.get("hypotheses", [])
    if not include_resolved:
        hypotheses = [h for h in hypotheses if not h.get("resolved")]
    return jsonify({
        "status": "success",
        "data": hypotheses,
    })
...
def post_hypothesis():
    ...
    required = ["subnet_id", "prediction", "predicted_delta_pct", "horizon", "current_price"]
    for field in required:
        if field not in body:
            return jsonify({"status": "error", "error": f"missing field: {field}"}), 400

    hypothesis = bridge.record_hypothesis(
        subnet_id=int(body["subnet_id"]),
        prediction=body["prediction"],
        predicted_delta_pct=float(body["predicted_delta_pct"]),
        horizon_iso=body["horizon"],
        current_price=float(body["current_price"]),
        sources=body.get("sources", []),
    )
    return jsonify({"status": "success", "data": hypothesis}), 201


@app.route("/api/mindmap/hypotheses/<int:subnet_id>/resolve", methods=["POST"])
def resolve_hypothesis(subnet_id):
    """Resolve the latest pending hypothesis for a subnet."""
    body = request.get_json(silent=True) or {}
    resolution_price = body.get("resolution_price")
    if resolution_price is None:
        return jsonify({"status": "error", "error": "resolution_price is required"}), 400

    resolved = bridge.resolve_hypothesis(subnet_id, float(resolution_price))
    if resolved is None:
        return jsonify({"status": "error", "error": "no pending hypothesis found"}), 404
    return jsonify({"status": "success", "data": resolved})


@app.route("/api/price-oracle/<token>", methods=["GET"])
def get_price_oracle(token):
    """Fetch current price for a token from the price oracle."""
    from internal.price_oracle import fetch_current_price
    result = fetch_current_price(token)
    if result is None:
        return jsonify({"status": "error", "error": f"no price data for {token}"}), 404
    return jsonify({"status": "success", "data": result})


@app.route("/api/price-oracle", methods=["GET"])
def get_price_oracle_all():
